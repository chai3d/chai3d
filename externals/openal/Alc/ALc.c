...
{
    OALEnterCriticalSection(&ListLock);
}
static void UnlockLists(void)
{
    OALLeaveCriticalSection(&ListLock);
}
...
BOOL APIENTRY DllMain(HINSTANCE hModule,DWORD ul_reason_for_call,LPVOID lpReserved)
{
    ...
    switch(ul_reason_for_call)
    {
        ...
        case DLL_THREAD_DETACH:
            ...
            {
                void *ptr = OAL_pthread_getspecific(TlsDestructor.array[i].key);
...
static void alc_init(void)
{
    ...
    OAL_pthread_key_create(&LocalContext, ReleaseThreadCtx);
    OALInitializeCriticalSection(&ListLock);
    ThunkInit();
}
...
#define DO_INITCONFIG() OAL_pthread_once(&alc_config_once, alc_initconfig)
...
static void alc_deinit_safe(void)
{
    ...
    ThunkExit();
    OALDeleteCriticalSection(&ListLock);
    OAL_pthread_key_delete(LocalContext);
...
static void alc_deinit(void)
{
    ...
    // 20161123 - added by seb for CHAI3D
    // static destructor is called before shared_ptr<cVideo> garbage collection,
    // which causes said cVideo destructor to crash on most platforms
    // since CHAI3D uses OpenAL as a static lib, skipping all deallocation should be of no consequence
    return;
    // 20161123 - end of CHAI3D mod
...
{
    OALEnterCriticalSection(&device->Mutex);
}
void ALCdevice_UnlockDefault(ALCdevice *device)
{
    OALLeaveCriticalSection(&device->Mutex);
}
...
static ALCvoid FreeDevice(ALCdevice *device)
{
    ...
    OALDeleteCriticalSection(&device->Mutex);
...
static void ReleaseContext(ALCcontext *context, ALCdevice *device)
{
    ...
    if(OAL_pthread_getspecific(LocalContext) == context)
    {
        WARN("%p released while current on thread\n", context);
        OAL_pthread_setspecific(LocalContext, NULL);
...
ALCcontext *GetContextRef(void)
{
    ...
    context = OAL_pthread_getspecific(LocalContext);
    if(context)
...
ALC_API ALCboolean ALC_APIENTRY alcMakeContextCurrent(ALCcontext *context)
{
    ...
    if((context=OAL_pthread_getspecific(LocalContext)) != NULL)
    {
        OAL_pthread_setspecific(LocalContext, NULL);
        ALCcontext_DecRef(context);
    }
...
ALC_API ALCboolean ALC_APIENTRY alcSetThreadContext(ALCcontext *context)
{
    ...
    /* context's reference count is already incremented */
    old = OAL_pthread_getspecific(LocalContext);
    OAL_pthread_setspecific(LocalContext, context);
...
ALC_API ALCdevice* ALC_APIENTRY alcOpenDevice(const ALCchar *deviceName)
{
    ...
    OALInitializeCriticalSection(&device->Mutex);
    device->LastError = ALC_NO_ERROR;
