...
ApplicationWidget::ApplicationWidget (QWidget *parent)
{
    ...
    m_camera->m_backLayer->addChild(m_background);

    // set background properties
    m_background->setCornerColors(cColorf(0.00f, 0.00f, 0.00f),
                                  cColorf(0.00f, 0.00f, 0.00f),
                                  cColorf(0.44f, 0.44f, 0.88f),
                                  cColorf(0.44f, 0.44f, 0.88f));

    // create a font
    m_font = NEW_CFONT_CALIBRI_20();

    // create a label
    m_label = new cLabel(m_font);
    m_camera->m_frontLayer->addChild(m_label);
    m_label->setText("use mouse to rotate camera");
    m_label->m_fontColor.setWhite();
};
...
void ApplicationWidget::resizeGL (int a_width,  int a_height)
{
    ...
    m_label->setLocalPos((int)(0.5 * (m_width - m_label->getWidth())), 20);
...
{
    double radius = m_camera->getSphericalRadius() + (double)(event->angleDelta().y()) * 0.001;
...
void ApplicationWidget::mouseMoveEvent(QMouseEvent *event)
{
    if (m_mouseMoveCamera)
    {
        ...
        double azimuthDeg = m_camera->getSphericalAzimuthDeg() - 0.2 * dx;
        double polarDeg = m_camera->getSphericalPolarDeg() - 0.2 * dy;
