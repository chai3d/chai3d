    ...
    for (int i=0;i<argCnt;i++)
    {
        ...
        if (p->inputArgTypeAndSize[i*2+0]==sim_lua_arg_nil)
        {
            if (int(_inData.size())<requiredArgumentCount)
                return(false);
            break; // this argument is nil, so it is like inexistant. But do also won't explore any more arguments, we have enough.
        }
        if (p->inputArgTypeAndSize[i*2+0]!=expectedArguments[1+i*2+0])
        {
